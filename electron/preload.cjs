...
contextBridge.exposeInMainWorld('agentDock', {
  ...
  getTokenUsageStats: () => ipcRenderer.invoke('usage:stats'),
  ...
  listRunArtifacts: (runId) => ipcRenderer.invoke('runs:list-artifacts', { runId }),
  getRunManifest: (runId) => ipcRenderer.invoke('runs:manifest', { runId }),
  verifyRunManifest: (runId) => ipcRenderer.invoke('runs:verify-manifest', { runId }),
  adoptRunPatch: (request) => ipcRenderer.invoke('runs:adopt-patch', request),
  rejectRunApproval: (request) => ipcRenderer.invoke('runs:reject-approval', request),
  ...
  readPlan: (request) => ipcRenderer.invoke('plan:read', request),
  verifyPlanHash: (request) => ipcRenderer.invoke('plan:verify-hash', request),
  adoptPlan: (request) => ipcRenderer.invoke('plan:adopt', request),
  getBudgetSpend: (request) => ipcRenderer.invoke('budget:spend', request),
  getSessionContextSummary: (sessionId) => ipcRenderer.invoke('context:summary', sessionId),
  getRunContextSnapshot: (runId) => ipcRenderer.invoke('context:snapshot', runId),
  ...
  runRace: (request) => ipcRenderer.invoke('agent:race', request),
  runCouncil: (request) => ipcRenderer.invoke('agent:council', request),
  stopAgent: (runId) => ipcRenderer.invoke('agent:stop', runId),
  getDelegateSubRunStatus: (subRunId) => ipcRenderer.invoke('delegate:status', { subRunId }),
  getDelegateSubRunResult: (subRunId) => ipcRenderer.invoke('delegate:result', { subRunId }),
  browser: {
